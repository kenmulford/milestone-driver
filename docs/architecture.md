...
| Hooks | `hooks/` | The eight shipped hooks (see `hooks/hooks.json`) are the seven `PreToolUse` gates - `force-subagent`, `no-bom`, `tests-green`, `no-push`, `no-pr-to-protected`, `code-review-gate`, `dispatch-cap` - plus `session-resume`, the one `SessionStart` hook (matcher `compact`) - all invoked via `hooks/run-hook.cmd` (bash-first, pwsh-fallback, fail-open). `session-resume` never gates: it re-injects the milestone run's wave-state checkpoint after an auto-compaction. The triage / declaration layers are procedural (skill-level), not hooks. See [The layered gating model](#the-layered-gating-model). |
...
**Known limitation under parallel builds (the default).** There is no per-step static-vs-port-binding classification - that is deferred, not implemented. Consequently, in a parallel run a CI-derived **server-starting** gating step (one that binds a port / starts a long-running server) is classified parallel-safe and run in the concurrent build phase, where it can contend for the same port as another worktree's build, instead of being deferred to the serial merge tail where port-binding gates belong. The mode does not detect or isolate this. Scope it out with the `ciWorkflow` override (narrow to a workflow with no server-starting gating step), or - since parallel is the default - set `parallel: false` (the opt-out) when your CI's PR-gating steps start a server and you use `preflightCmd: "github-ci"`. This is not handled automatically.
...
So concurrency is by stage, not by issue. The orchestrator fans out one leaf per issue to implement, fans out that issue's reviewer as soon as its implementer returns, then runs the gates, the version bump, the commit, and the PR itself. The two stages overlap in time - a fast issue's review runs while a slow one is still building - but the invariant constrains depth, not ordering across issues: the expensive stages stay concurrent, and nothing ever dispatches at depth 2.

The `/code-review` gate is untouched by this. It stays mandatory and unconditional; the orchestrator runs it and dispatches the reviewers directly as leaves. Only who launches them, and at what depth, changed.

The corollary the engine already relies on everywhere: **terminal state is derived from git and gh artifacts, never from an agent's self-report.** A perfectly formed status field proves only what the agent believed, so a schema on top of a self-report buys nothing. The probe reads artifacts.

## Parallel mode

By default the milestone loop builds the mutually-independent issues within a single Wave concurrently, each in its own git worktree, then integrates them through one orchestrator-owned serial verified merge tail. Parallel is the default execution mode; a run drops to the one-issue-at-a-time sequential path (single working tree, no worktrees) only when a barrier is present at run start. The standing opt-out is the `parallel: false` profile key.

The execution mode is resolved **once at run start** by a "Resolve execution mode" barrier cascade (a fixed, top-down list of checks - the first one that matches decides, and the rest are skipped) - not parsed from a CLI flag. Claude Code does no argument parsing, so there is no mode token to type; the cascade is evaluated top-down, first match wins, and the resolved mode holds for the whole run:

1. `parallel: false` in the profile → **sequential** - the standing opt-out (quiet).
2. A permission-allowlist gap (the existing pre-flight gate) → **sequential, with synchronous dispatch**. Concurrent leaf dispatch requires background dispatch, which the gap forbids, so the run also drops to sequential. The gate surfaces the gap and recommends `/fewer-permission-prompts`.
3. `parallel: true` in the profile → **parallel** - the operator has asserted per-worker isolation is safe (quiet).
4. `unitTestCmd` is set and `parallel` is absent, in an **interactive** run → the one-time **DB-hazard interview** (below) decides, and its answer is **persisted** to the `parallel` key so the question is asked once.
5. The same case in a **non-interactive** run (`MILESTONE_DRIVER_NONINTERACTIVE=1`) → **sequential**, with a loud note on how to set `parallel: true` to enable parallel builds. No value is persisted - no human actually decided.
6. Otherwise → **parallel**, the default.

So an **absent** `parallel` key means "not yet decided", not "off": the run is parallel unless `unitTestCmd` triggers the interview. The resolved mode is surfaced in the run output.

A habit-typed `--parallel` token is harmlessly stripped by the generic argument resolver and ignored - parallel is already the default - so it is a no-op, not an error.
...
2. Dispatch by stage, in a worktree fleet. The orchestrator owns worktree creation: it creates one git worktree per set issue (under the gitignored scratch dir `.milestone-config/worktrees/`) and drives each issue's pipeline itself, inside that worktree. It dispatches one implementer subagent per issue concurrently, and dispatches each issue's reviewer the moment that issue's implementer returns, so builds and reviews overlap rather than queueing behind the slowest build. Dispatch is capped at 4 concurrent agents by default - **one shared in-flight counter spanning both stages**, not one window per stage - the default of the tunable `maxParallelWorkers` profile key (integer, optional, default 4; when set, it overrides the hardcoded cap). The cap is orthogonal to `parallel` - `parallel` decides *whether* to parallelize, `maxParallelWorkers` decides *how wide* - and has no effect on a sequential run. A freed slot goes to the oldest review-ready issue, and starts a new implementer only when none is waiting, so the in-flight count never exceeds the cap.
3. Wave-level gather, then derive terminal state from artifacts. This is Phase 1's barrier (the Wave's other one is the Wave boundary itself, below): the Wave does not advance and integration does not begin until every dispatched agent has returned. The orchestrator then derives each issue's state from git and gh rather than from what any agent said: built-green (branch built, verified, pushed) or parked (comment plus label plus preserved branch, excluded from integration). Those two are not exhaustive on their own. An interrupted, auto-denied, or killed run can leave an issue in neither, so the probe names that third state, abandoned, and the orchestrator recovers it exactly once before parking blocked.

The expensive stages, implementation and review, are the concurrent ones: they pipeline against one shared in-flight counter rather than running one after the other. The gates, the version bump, the commit, and the PR are the orchestrator's own, run per issue in that issue's worktree: everything the per-issue pipeline does in a sequential run, at the same gates and under each gate's own cap (at most 2 for unit, E2E, and preflight; step 6.1's classifier verdict for the review gate), with only the dispatch shape different. E2E and any server-starting preflight are deferred to the serial merge tail, where they run once against accumulated state.

### Serial verified merge tail

Phase 1 - the concurrent stage dispatch covered above - builds but does not integrate. Phase 2, the serial verified merge tail, integrates the gathered built-green branches one at a time, in ascending-issue order, on the main working tree. It is force-free by default (merge-in, no history rewrite). For each branch the orchestrator merges the integration target into the issue branch as an ordinary merge commit (a fast-forwardable push, no `--force`), re-verifies against accumulated state (the unit suite if defined, plus the E2E and server-starting preflight gates Phase 1 deferred, run once here against the integrated result where they are more meaningful), and on green squash-merges the PR. The squash collapses the merge-in commit so the integration target's history stays linear. The integration target advances after each merge, so two same-Wave siblings that touch overlapping files are re-verified against each other, restoring the "every increment tested against accumulated state" guarantee that naive concurrent merging throws away.
...
`--driven` is an internal token, recognized by string presence in the invocation text, never typed by a human and never a parsed CLI flag. The fan-out loop supplies it every time it invokes `/milestone-driver:solve-milestone <number> --driven` on its own behalf. `--driven` does two things to that invocation:
...
`"milestone"` exists for one problem: a repo whose CI fires on a push to **every** branch, where a milestone's worth of issue branches exhausts the available GitHub runners or container capacity. `"wave"` does not fix that problem, because a wave still pushes one branch per issue and the wave branch is assembled from those already-pushed branches. Wave granularity cuts the PR and CI count to O(waves) but leaves the push count at O(issues). `"milestone"` is the value that keeps every intermediate branch local, so a whole milestone costs one push.
