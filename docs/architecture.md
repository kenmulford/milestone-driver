...
The execution mode is resolved **once at run start** by a "Resolve execution mode" barrier cascade — not parsed from a CLI flag. Claude Code does no argument parsing, so there is no mode token to type; the cascade is evaluated top-down, first match wins, and the resolved mode holds for the whole run:
