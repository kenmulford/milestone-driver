...
2. A permission-allowlist gap (the existing pre-flight gate) → **sequential, with synchronous dispatch**. Parallel workers require background dispatch, which the gap forbids, so the run also drops to sequential. The gate surfaces the gap and recommends `/fewer-permission-prompts`.
