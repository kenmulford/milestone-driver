...
The expensive stage (implementation) is the concurrent one. The gates, the version bump, the commit, and the PR are the orchestrator's own, run per issue in that issue's worktree: everything the per-issue pipeline does in a sequential run, at the same gates and the same at-most-2 caps, with only the dispatch shape different. E2E and any server-starting preflight are deferred to the serial merge tail, where they run once against accumulated state.
