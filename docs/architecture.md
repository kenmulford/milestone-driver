...
So concurrency is by stage, not by issue. The orchestrator fans out one leaf per issue to implement, barriers, fans out one leaf per issue to review, then runs the gates, the version bump, the commit, and the PR itself. The expensive stage stays concurrent, and nothing ever dispatches at depth 2.
...
2. Dispatch by stage, in a worktree fleet. The orchestrator owns worktree creation: it creates one git worktree per set issue (under the gitignored scratch dir `.milestone-config/worktrees/`) and drives each issue's pipeline itself, inside that worktree. It dispatches one implementer subagent per issue concurrently, barriers on them, then dispatches one reviewer per issue concurrently. Dispatch is capped at 4 concurrent agents per stage by default — the default of the tunable `maxParallelWorkers` profile key (integer, optional, default 4; when set, it overrides the hardcoded cap for the rolling window). The cap is orthogonal to `parallel` — `parallel` decides *whether* to parallelize, `maxParallelWorkers` decides *how wide* — and has no effect on a sequential run. A larger set uses a rolling window so the in-flight count never exceeds the cap.
3. Barrier, then derive terminal state from artifacts. The Wave does not advance and integration does not begin until every dispatched agent has returned. The orchestrator then derives each issue's state from git and gh rather than from what any agent said: built-green (branch built, verified, pushed) or parked (comment plus label plus preserved branch, excluded from integration). Those two are not exhaustive on their own. An interrupted, auto-denied, or killed run can leave an issue in neither, so the probe names that third state, abandoned, and the orchestrator recovers it exactly once before parking blocked.
