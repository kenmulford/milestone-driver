...
**Sentinel mode (`preflightCmd: "github-ci"`).** The reserved sentinel `"github-ci"` runs no CI step locally: no discovery, no local execution, no cap, no park at this gate. The PR's own CI run is the preflight instead - `solve-issue` step 8's existing wait-for-green merge, with that granularity's existing red-CI handling, is what gates the change. `scripts/ci-preflight-steps.{sh,ps1}` (a line-oriented `.github/workflows/*.yml` parser, no YAML library, no network call) still exists but nothing in this pipeline calls it. See `skills/solve-issue/preflight-github-ci.md` for the sentinel record.
