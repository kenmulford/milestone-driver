...
> **Enforcement model for `/code-review`:** Review-before-commit is enforced by **audit trail plus a mechanical gate**. `solve-issue` treats an omitted `/code-review` run as a **park trigger** — it comments the reason on the issue, applies the `blocked` label, and returns (the milestone loop continues); the omission is never silently accepted. The PR body requires a mandatory `## Code Review` section carrying a `/code-review run:` verdict, and the `code-review-gate` hook (see `hooks/hooks.json` — the shipped gates are `force-subagent`, `no-bom`, `tests-green`, `no-push`, `no-pr-to-protected`, `code-review-gate`) mechanically blocks `gh pr create`/`gh pr merge` on a missing section **and on a verdict that is not `yes`, `deferred (<reason>)`, or `n/a - <reason>`**: `no`, an unrecognized value, an empty value after `run:`, and a missing `run:` slot each deny, so heading presence alone no longer passes the gate. The verdict is read only from the text after the section's heading, so an incidental `/code-review run:` in prose above it is never taken as the verdict. A command targeting `protectedBranch` is exempt, so Ken's manual release-PR flow is never gated. A missing verdict is a body defect and denies; a broken environment is not, and `CLAUDE_HOOK_DISABLE_CODE_REVIEW_GATE=1`, a missing `jq` or `gh`, unparsed hook stdin, an unreadable `--body-file`, and a failed `gh pr view` all still fail open. Consumers should still inspect the Code Review section as part of their release checklist before approving the `integrationBranch` → `protectedBranch` merge.
