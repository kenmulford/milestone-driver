...
| `integrationGranularity` | `"issue" \| "wave" \| "milestone"` (string enum) | Integration | How should built issues integrate? Under `solve-milestone`, absent or unrecognized resolves to `"milestone"`, an unrecognized value with a logged note, never an error. Under a standalone `solve-issue` run, absent resolves to `"issue"`. `"issue"` - each built issue gets its own PR → CI → merge. `"wave"` - a whole Wave integrates on one branch `wave/<milestone>-w<N>` → one PR → one CI run. `"milestone"` → a whole milestone integrates on one local branch `milestone-<number>-<slug>`: one push → one PR → one CI run. | - |
