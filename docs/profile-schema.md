...
| **Integration** | `integrationGranularity`, `visualHold` | Optional |
...
| `visualHold` | boolean | Integration | Should the single milestone PR wait for a human to sign off on the UI before it merges? Read only under `integrationGranularity: "milestone"`. Absent (the default) → hold. `false` → the milestone PR auto-merges on green CI even when the branch touched a UI surface. A non-boolean value holds, with a logged note. | — |
...
- **Present but missing a required sub-key → two layers, two behaviors.** A `visualCapture` node present but missing `serverCmd`, `readyUrl`, or `signInPath` is handled differently by the two layers that touch it, and it matters which:
  - **The visual-capture flow/gate (`solve-issue` step 6.7, built by #210)** treats the incomplete block as *not configured* and degrades to the PR-open-for-human-test note — the run never fails and a UI issue never auto-merges, mirroring the `integrations.trello`-without-`boardId` graceful-degrade rule.
...
**Note on `visualHold`.** `visualHold` decides whether the single milestone PR waits for a human to look at the UI before it merges.

- **Applies only under `integrationGranularity: "milestone"`.** Under `"issue"` and `"wave"` the key has no effect and today's per-issue visual-review gate is untouched.
- **Absent means hold.** This is the **omit-the-default** convention `versioning` and `integrationGranularity` already follow: omit the key to get the default, write it only to override.
- **Fail toward holding.** When the milestone branch's diff against `integrationBranch` cannot be determined, the gate **holds** rather than auto-merges. A UI hold is a one-way door, so silently weakening it is unrecoverable, while an over-strict hold costs one operator action.
- **`visualHold: false` is the sole override**, and there is no invocation token for it. It disables the gate outright, bypassing both the UI-touch hold and the indeterminate-diff hold; fail-toward-holding governs only when `visualHold` is unset.
- **A non-boolean value fails open toward holding**, with a logged note, never a hard error, the same fail-open shape `maxParallelWorkers` and `versioning` already use. Resolved once at `solve-milestone`'s Before-starting profile read.
- **No `uiSurfaceGlobs` → nothing to hold for.** With `uiSurfaceGlobs` absent the repo has no UI surfaces, so the gate has nothing to hold on, exactly as with today's per-issue gate.
