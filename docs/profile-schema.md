...
> **See also:** [the layered gating model](architecture.md#the-layered-gating-model) - how `uiSurfaceGlobs` drives the design-lens triage - and [consumer setup](consumer-setup.md) for the run flow these keys feed.
...
Keep it minimal and consumer-driven. **Three keys are required** (`integrationBranch`, `protectedBranch`, `sourceGlobs`); the agent keys `implementerAgent`, `triageAgent`, `designReviewAgent`, `blockerResolverAgent`, and `coherenceReviewAgent` are default-filled (they default to `milestone-driver:implementer`, `milestone-driver:triage-reviewer`, `milestone-driver:design-reviewer`, `milestone-driver:blocker-resolver`, and `milestone-coherence-reviewer:coherence-reviewer`) so a profile may omit them. The first four resolve to always-on bundled agents; `coherenceReviewAgent`'s default points at the separate milestone-coherence-reviewer companion plugin, so its post-build coherence pass runs only when that companion is installed (absent → silently skipped). All other keys are optional. New keys are added only when a real second consumer needs them - never speculatively.
...
| **Core** (default-filled) | `implementerAgent`, `triageAgent`, `designReviewAgent`, `blockerResolverAgent`, `coherenceReviewAgent` | optional in file (auto-filled) |
...
| **Enrichment** | `domainSkills`, `nonNegotiables`, `projectDocs`, `standingDocs` | Optional |
| **External integrations** | `integrations.trello` | Optional |

**Note on safety keys:** `integrationBranch`, `protectedBranch`, and `sourceGlobs` are required for safe operation. The hooks fail-open when they are absent (a robustness measure so a hook bug never bricks a repo), but that fail-open is not a statement of optionality - without these keys the safety guarantees do not hold. `implementerAgent`, `triageAgent`, `designReviewAgent`, and `blockerResolverAgent` have bundled defaults (`milestone-driver:implementer`, `milestone-driver:triage-reviewer`, `milestone-driver:design-reviewer`, `milestone-driver:blocker-resolver`) and are auto-filled by the bootstrap; omitting them from the profile is valid and common. `coherenceReviewAgent` is also default-filled (`milestone-coherence-reviewer:coherence-reviewer`), but unlike the four above it points at a separate companion plugin: its post-build coherence pass runs only when the milestone-coherence-reviewer companion is installed, and is silently skipped otherwise (absent-means-skip).
...
| `blockerResolverAgent` | string | Core | Which agent decides whether a triage Blocker is already answered by the record before the issue parks (Step 3.5)? Default: `milestone-driver:blocker-resolver` (auto-filled; rarely overridden) | default-filled |
| `coherenceReviewAgent` | string | Core | Which agent runs the read-only post-build coherence pass before the final `/code-review`? Default: `milestone-coherence-reviewer:coherence-reviewer`. Default-filled; the pass runs only when the coherence-reviewer is present and configured, and is silently skipped otherwise (absent-means-skip). **Unlike `implementerAgent` / `triageAgent` / `designReviewAgent` / `blockerResolverAgent` - always-on bundled agents - this agent ships in the separate milestone-coherence-reviewer companion plugin, so the pass runs only when that companion is installed; absent → silently skipped.** | default-filled |
...
| `uiSurfaceGlobs` | string[] | Triage / Visual | Which path patterns mark UI surfaces? Drives `design-reviewer` dispatch (triage) and guards visual capture, e.g. `["PrayerApp/Views/**","**/*.xaml"]`. Absent → no design-lens review and no visual capture. | - |
| `visualCapture` | object | Visual capture | Visual-capture render seam - declares how an automated flow boots a seeded/persona app and what it captures. Presence = enabled; absence = skip (absent-means-skip, same convention as `unitTestCmd` / `integrations.trello`). When present, `serverCmd`, `readyUrl`, and `signInPath` are required; a node missing any of them is treated as absent with a one-line misconfiguration log. Absent → no daemon boots, no capture is attempted, and no gate is added. | - |
| `visualCapture.serverCmd` | string | Visual capture | Command that boots the seeded/persona app server (read by the render daemon, `scripts/render-daemon.sh ('.visualCapture.serverCmd // empty')`). **Required when the `visualCapture` node is present**; a node missing it is treated as absent + logged. | - |
| `visualCapture.readyUrl` | string | Visual capture | The full `/health`-style URL the ready probe polls to know the server is up (read by the render daemon, `scripts/render-daemon.sh ('.visualCapture.readyUrl // empty')`), e.g. `http://127.0.0.1:3000/health`. **Required when present**; missing → node treated as absent + logged. | - |
...
| `preflightCmd` | string | Preflight | Either a single literal command that runs your project's fast pre-PR checks (lint, format, static analysis, security scan), **or** the reserved sentinel `"github-ci"` which defers entirely to your GitHub Actions CI - no local check runs; the PR's own CI run is the preflight (see below). Absent → preflight gate skipped cleanly. A literal command runs after `/code-review`, before commit. | - |
| `ciWorkflow` | string | Preflight | Retained for a future re-introduction of local CI-derived discovery; not read by anything today. `preflightCmd: "github-ci"` runs no local discovery to narrow. | - |
| `integrationGranularity` | `"issue" \| "wave" \| "milestone"` (string enum) | Integration | How should built issues integrate? Under `solve-milestone`, absent or unrecognized resolves to `"milestone"`, an unrecognized value with a logged note, never an error. Under a standalone `solve-issue` run, absent or unrecognized resolves to `"issue"`. `"issue"` - each built issue gets its own PR → CI → merge. `"wave"` - a whole Wave integrates on one branch `wave/<milestone>-w<N>` → one PR → one CI run. `"milestone"` → a whole milestone integrates on one local branch `milestone-<number>-<slug>`: one push → one PR → one CI run. | - |
...
| `maxParallelWorkers` | integer | Execution | The per-Wave concurrent-agent cap - how many agent leaves run at once across build and review together. Optional, **default 4**; omit to get 4, set only to override. Absent or invalid (non-integer, `< 1`) → 4 (fail-open, never an error). Orthogonal to `parallel` (whether vs. how wide); no effect on a sequential run. See the note below. | - |
| `versioning` | boolean | Release | Should each PR bump a plugin version? `false` → version-free: no extraction, no prompt, no bump (the milestone title need not be a version). absent (default) → opportunistic: `solve-milestone` runs the deterministic extractor (`scripts/extract-version.*`) against the milestone title (description as fallback); a parseable version is used, otherwise the run silently degrades to version-free with a logged note - never prompts. `true` (explicit opt-in) → same extraction, but a miss or an ambiguous title prompts the operator (or, under `MILESTONE_DRIVER_NONINTERACTIVE=1`, degrades with a loud warning). This `absent` vs `true` split is intentional: `true` asserts intent to version, so a missing version is treated as a likely misconfiguration. Fail-safe: in versioned mode, a missing `.claude-plugin/plugin.json` degrades to version-free with a logged note rather than failing. | - |
| `domainSkills` | string[] | Enrichment | Exact `plugin:skill` names the implementer invokes with the Skill tool for citations (e.g. `["maui-skills:maui-data-binding", "maui-skills:maui-shell-navigation"]`), and that the `triage-reviewer` and `design-reviewer` invoke as one step - after the framework's own docs, before repo patterns - in their research path when verifying a convention they found is a genuine framework idiom rather than a merely-local repo habit. Absent → general docs + repo conventions only. A `<plugin>:*` entry is expanded to exact names by `scripts/expand-domain-skills.{sh,ps1}` before any consumer sees it; setup records only the expansion, so a `*` never reaches this file. | - |
...
| `standingDocs` | string[] | Enrichment | Anchors shaped `<doc>#<heading>`, `<doc>` a path under `projectDocs`, that `scripts/assemble-common.{sh,ps1}` quotes into `.milestone-config/.runtime/plans/common.md` once per run, after the plugin's contract text. Absent → `common.md` carries the plugin contract text only. | - |
...
**Note on `uiSurfaceGlobs`.** `uiSurfaceGlobs` drives two procedural (skill-level) steps, neither a mechanical hook: design-lens triage (`design-reviewer`), and the guard on visual capture, which runs only when the PR's diff matches these globs. Triage reviews the *recorded design + source*, so it needs no render capability; screenshot capture does, and that capability is the `visualCapture` seam (documented below). When `uiSurfaceGlobs` is absent, the repo has no UI surfaces: no design-lens review and nothing for capture to shoot. See [the layered gating model](architecture.md#the-layered-gating-model) for the two-layer model these keys participate in.

**Note on `visualCapture`.** `visualCapture` is the dedicated render-capability seam for screenshot capture - the object-valued, optional render-capability declaration the note above points at. It is shaped on `e2eEnv` (an object-valued optional key) and mirrors the present/absent + sparse-optional-write conventions of `integrations.trello`; note that a *missing required* sub-key disables the whole block (unlike trello's optional-list override). With that one difference flagged:

This block is a web/HTTP-server-boot + URL-polling shape (`serverCmd` boots a server, `readyUrl` polls it); native UI stacks (MAUI, WPF, and similar) have no server or URL to poll and should omit `visualCapture` entirely; their PRs then carry the diff alone, with no screenshots attached.

- **Absent block → skip.** When `visualCapture` is absent, no daemon boots and no capture is attempted - no new gate, no prompt, no error (absent-means-skip, the same convention as `unitTestCmd` / `integrations.trello`).
- **Present but missing a required sub-key → two layers, two behaviors.** A `visualCapture` node present but missing `serverCmd`, `readyUrl`, or `signInPath` is handled differently by the two layers that touch it, and it matters which:
  - **The visual-capture flow (`solve-issue` step 6.8)** treats the incomplete block as *not configured* and skips capture - the run never fails and the PR proceeds exactly as it would with no capture configured, mirroring the `integrations.trello`-without-`boardId` graceful-degrade rule.
  - **The render daemon itself fails loud** if it is driven with an incomplete block: `read_profile` exits nonzero (`read_profile || exit 2`) with `render-daemon: profile is missing visualCapture.serverCmd and/or visualCapture.readyUrl` when either of the two keys it reads is empty (`scripts/render-daemon.sh (-z "$SERVER_CMD")`, `scripts/render-daemon.ps1 (profile is missing)`; each `start` branch aborts on that result at `scripts/render-daemon.sh (read_profile ||)` and `scripts/render-daemon.ps1 (if (-not (Read-Profile)))`). The daemon halts rather than silently mis-rendering, so the misconfiguration surfaces.
  In short: the capture flow degrades gracefully; the daemon, if invoked directly with a bad block, halts loudly. There is no path that silently captures the wrong thing.
...
**Required vs optional sub-keys.** Required-when-present: `serverCmd`, `readyUrl`, `signInPath`. Optional-with-default: `persona` (`"super-admin"`), `viewports` (`{ "desktop": { "width": 1440, "height": 900 } }`), `appearances` (`["light"]`). The two keys the render daemon consumes are `serverCmd` and `readyUrl` (`scripts/render-daemon.sh ('.visualCapture.serverCmd // empty')`); `signInPath`, `persona`, `viewports`, and `appearances` are read by the capture consumer (`solve-issue` step 6.8), not the render daemon. Sparse-write shape: a block written with optional sub-keys at their defaults stores only the keys actually supplied (the same sparse-object rule as `integrations.trello.lists`) - omitted optional sub-keys are not written and resolve to their defaults at runtime.
...
- **Orthogonal to `parallel`.** `parallel` decides *whether* to parallelize; `maxParallelWorkers` decides *how wide*. It has no effect on a sequential run (which builds one issue at a time regardless of the cap). `solve-milestone`'s Phase-1 shared-slot dispatch reads this value - see `### Parallel mode - Phase 1: concurrent stage dispatch` for the mechanics.
...
**Preflight sentinel (`preflightCmd: "github-ci"`).** Set `preflightCmd` to the reserved sentinel `"github-ci"` to defer the preflight gate entirely to your GitHub Actions CI: no local check runs, no discovery, no cap, no park at this gate. The PR's own CI run is the preflight - `solve-issue` step 8's existing wait-for-green merge, with that granularity's existing red-CI handling (the per-issue PR's wait, the wave PR's, or the milestone PR's), is what gates the change. The literal-command and absent-`preflightCmd` paths are unaffected by the sentinel; `"github-ci"` is mutually exclusive with a literal command. `ciWorkflow` is not read under this sentinel - see its own row above. `scripts/ci-preflight-steps.{sh,ps1}`, which reads `.github/workflows/*.yml` and emits runnable `run:` steps, still exists but nothing in this pipeline invokes it.

**Note on `integrationGranularity` and wave granularity.** Under a standalone `solve-issue` run, absent resolves to `"issue"` and integrates per issue: each built issue opens its own PR → its own CI run → merges individually. Under `solve-milestone`, absent resolves to `"milestone"` instead (see the row above). Set `integrationGranularity: "wave"` explicitly for a repo with long or expensive CI, where O(issues) per-issue CI runs are wasteful: a whole dependency Wave integrates on one branch `wave/<milestone>-w<N>`, opens one wave PR → `integrationBranch`, and runs one CI run for the assembled Wave - trading O(issues) CI runs for O(waves), and CI-validating the *assembled* Wave (catching integration-level issues an isolated per-issue build misses). The merge-tail mechanism (merge-in + re-verify against accumulated state + bounded auto-resolve) is the same as under `"issue"`; only the target (the wave branch instead of `integrationBranch`) and PR-opening (one wave PR instead of per-issue PRs) differ. Trade-off: one red wave-PR CI blocks the whole Wave - acceptable because the strong local gates (unit + static preflight + `/code-review` + the tail's re-verify) catch most failures before CI; CI is the backstop. Not for repos with weak local gates. Orthogonal to execution mode (the `parallel` key) (which is *how* issues build; this is *how* they integrate) - the two combine or apply independently. See `skills/solve-milestone/parallel-waves.md § Integration granularity (issue vs wave)` for the orchestrator mechanics, and the Integration tier of `skills/setup/SKILL.md` (Phase 2) for the setup-time selection step - where choosing `"wave"` fires the non-blocking precondition prompt about local-gate strength.

**Note on `integrationGranularity` and milestone granularity.** Under `solve-milestone`, absent resolves to `"milestone"` (see the row above). Under a standalone `solve-issue` run, absent resolves to `"issue"` and integrates per issue: each built issue opens its own PR, gets its own CI run, and merges individually. Milestone granularity is the fit for a repo whose CI triggers on a push to every branch, where one branch per issue exhausts the available GitHub runners and containers. `"wave"` does not solve that, because wave granularity still pushes a branch per issue; only milestone granularity keeps every intermediate branch local. Under `"milestone"`, a whole milestone builds on one local branch `milestone-<number>-<slug>`, cut from `integrationBranch`. Issue branches keep their `issue/<n>-<slug>` naming but are cut from the milestone branch and are never pushed; each finished issue is folded in as one local squash commit carrying an `Issue: #<n>` trailer. The milestone branch is pushed once, at milestone end, and integrates into `integrationBranch` through one PR and one CI run. A second payoff: a dependency chain builds straight through locally, with no round trip to origin per link. Trade-off: nothing reaches origin until that milestone-end push, so remote CI first runs on the assembled milestone and the local gates (unit, preflight, `/code-review`) are the earlier signal, the same bargain wave granularity makes at Wave scope. The `milestone-` branch prefix is a stable, externally-consumed contract: consumers write their own CI branch filters against it, so renaming it later silently un-excludes every consumer's CI. Applies under both sequential and parallel execution, exactly as wave granularity does, and stays orthogonal to execution mode (the `parallel` key): `parallel` is *how* issues build, `integrationGranularity` is *how* they integrate. An out-of-enum `integrationGranularity` value fails open to `"milestone"` with a logged note, never a hard error, resolved once at `solve-milestone`'s Before-starting profile read.

**Note on execution mode (parallel by default).** Execution mode is not a run argument - there is no `--parallel` flag and no "in parallel" trigger. Parallel is the default mode for `solve-milestone`: it builds the mutually-independent issues within a Wave concurrently, each in its own git worktree, then integrates them through the serial verified merge tail. At run start `solve-milestone` resolves the mode once, through a barrier cascade, and drops to sequential only when a barrier is present - a `parallel: false` opt-out, a permission-allowlist gap (which forces synchronous, sequential dispatch), or an unresolved test-isolation hazard when `unitTestCmd` is set and `parallel` is absent (the one-time DB-hazard interview). The standing control is the `parallel` profile key (above), not a flag. Concurrency is capped at 4 agent leaves per Wave by default - one shared cap across the concurrent build and the concurrent review, which overlap - now tunable via the `maxParallelWorkers` key (above) rather than being hardcoded. Execution mode is orthogonal to `integrationGranularity`: the former is *how* issues build, the latter is *how* they integrate. A habit-typed `--parallel` token on the invocation is harmlessly stripped and ignored (parallel is already the default). See `solve-milestone`'s Resolve execution mode Before-starting step, `### Parallel mode - Phase 1: concurrent stage dispatch`, and `### Integration granularity (issue vs wave)` for the mechanics.
...
The default-filled agent keys (`implementerAgent`, `triageAgent`, `designReviewAgent`, `blockerResolverAgent`, `coherenceReviewAgent`) are omitted here; their defaults apply automatically. The first four are bundled agents that always resolve; `coherenceReviewAgent`'s default is the milestone-coherence-reviewer companion, so its pass runs only when that companion plugin is installed (absent → silently skipped).
...
```json
{
  ...
  "domainSkills": ["maui-skills:maui-data-binding", "maui-skills:maui-shell-navigation"],
  "nonNegotiables": [
...
These three keys satisfy the consumer-driven rule above with a real consumer, not speculation: PracticingPrayer (consumer #1) uses `uiSurfaceGlobs` for its XAML views (shown above); `triageAgent` and `designReviewAgent` are auto-filled, so the profile omits them while the bundled triage phases consume their defaults. PracticingPrayer runs version-free (`versioning: false`) because its version lives in the `.csproj`, not a `plugin.json`, so the loop bumps nothing and its milestones need no semver name.
...
| `dispatch-cap` (PreToolUse `Agent`/`Task`/`Skill`) | `implementerAgent` (default-filled; names the counted agent) |
...
> **Enforcement model for `/code-review`:** Review-before-commit is enforced by audit trail plus a mechanical gate. `solve-issue` treats an omitted `/code-review` run as a park trigger: it comments the reason on the issue, applies the `blocked` label, and returns (the milestone loop continues); the omission is never silently accepted. The PR body requires a mandatory `## Code Review` section carrying a `/code-review run:` verdict, and the `code-review-gate` hook (see `hooks/hooks.json`; the shipped gates are `force-subagent`, `no-bom`, `tests-green`, `no-push`, `no-pr-to-protected`, `code-review-gate`, `dispatch-cap`) mechanically blocks `gh pr create`/`gh pr merge` on a missing section and on a verdict that is not `yes` or `n/a - <reason>`: `no`, an unrecognized value, an empty value after `run:`, and a missing `run:` slot each deny, so heading presence alone no longer passes the gate. `n/a` requires a PR carrying no `sourceGlobs` change; there is no third accepted value, because an omitted `/code-review` parks the issue and a parked issue opens no PR. The verdict is read only from the text after the section's heading, so an incidental `/code-review run:` in prose above it is never taken as the verdict. A command targeting `protectedBranch` is exempt, so Ken's manual release-PR flow is never gated. A missing verdict is a body defect and denies; a broken environment is not, and `CLAUDE_HOOK_DISABLE_CODE_REVIEW_GATE=1`, a missing `jq` or `gh`, unparsed hook stdin, an unreadable `--body-file`, and a failed `gh pr view` all still fail open. Consumers should still inspect the Code Review section as part of their release checklist before approving the `integrationBranch` → `protectedBranch` merge.
