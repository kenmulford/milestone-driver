...
| `visualCapture.aiPrefilter` | boolean | Visual capture | Opt-in AI pre-filter over the captured screenshots - after the Publish substep, the orchestrator reads each captured PNG and posts all per-surface/viewport/appearance `pass` / `suspected-issue` verdicts in ONE separate PR comment. Optional; default absent/`false` → pre-filter skipped with one log line. Never a merge gate - a UI issue auto-merges on green CI regardless of verdicts. | - |
...
| `maxParallelWorkers` | integer | Execution | The per-Wave concurrent-agent cap - how many agent leaves run at once across build and review together, overriding the previously-hardcoded 4. Optional, **default 4**; omit to get 4, set only to override. Absent or invalid (non-integer, `< 1`) → 4 (fail-open, never an error). Orthogonal to `parallel` (whether vs. how wide); no effect on a sequential run. See the note below. | - |
...
**Required vs optional sub-keys.** Required-when-present: `serverCmd`, `readyUrl`, `signInPath`. Optional-with-default: `persona` (`"super-admin"`), `viewports` (`{ "desktop": { "width": 1440, "height": 900 } }`), `appearances` (`["light"]`), `aiPrefilter` (`false` - pre-filter skipped). The two keys the render daemon (#208) consumes are `serverCmd` and `readyUrl` (`scripts/render-daemon.sh ('.visualCapture.serverCmd // empty')`); `signInPath`, `persona`, `viewports`, `appearances`, and `aiPrefilter` are read by the capture consumer (`solve-issue` step 6.8), not the render daemon. **Sparse-write shape:** a block written with optional sub-keys at their defaults stores only the keys actually supplied (the same sparse-object rule as `integrations.trello.lists`) - omitted optional sub-keys (an omitted `aiPrefilter` included) are not written and resolve to their defaults at runtime.
