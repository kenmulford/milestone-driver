...
| `domainSkills` | string[] | Enrichment | Stack-specific skill identifiers the implementer consults for citations (e.g. `["maui-skills:*"]`), and that the `triage-reviewer` and `design-reviewer` consult as one step — after the framework's own docs, before repo patterns — in their research path when verifying a convention they found is a genuine framework idiom rather than a merely-local repo habit. Absent → general docs + repo conventions only. | — |
