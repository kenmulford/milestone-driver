...
| `integrationGranularity` | `"issue" \| "wave" \| "milestone"` (string enum) | Integration | How should built issues integrate? Default `"issue"` - each built issue gets its own PR → CI → merge. `"wave"` - a whole Wave integrates on one branch `wave/<milestone>-w<N>` → one PR → one CI run. `"milestone"` → a whole milestone integrates on one local branch `milestone-<number>-<slug>`: one push → one PR → one CI run. An unrecognized value resolves to the default `"issue"` with a logged note, never an error. | - |
