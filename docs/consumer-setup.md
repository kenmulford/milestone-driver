...
- **Preflight gate (post-build, before the PR).** If you set `preflightCmd` in your profile to a literal command, the run executes your fast pre-PR checks locally at the end of the code-review loop (before the PR opens), so a lint / static-analysis / security failure is caught and fixed up front instead of turning the PR red. CI remains the authority - this just surfaces a red result earlier. `preflightCmd` also accepts the reserved sentinel `"github-ci"`, which runs no local check at all: the PR's own CI run is the preflight instead, gated by the existing wait-for-green merge (see `docs/profile-schema.md`). Absent → skipped. First-run notice: on the first `solve-issue` / `solve-milestone` run where `preflightCmd` isn't set in your profile, the run prints a one-time, plain-English notice introducing it - this mostly matters when upgrading from 1.3.x, whose existing profile means `setup` won't re-run to offer the key. It shows at most once per clone (marker `.milestone-config/preflight-notice`, gitignored) and is silent once `preflightCmd` is set.
