...
<!--
Part of your project docs (.project/). Tools read and cite this file by its
path plus `#` and a `##` heading's exact text. Fill every [TBD]. A section left as
...
> Two defense-in-depth gating **layers** catch design gaps before they reach the integration branch (`docs/architecture.md#The layered gating model`): Layer 0, proactive triage (design gaps + dependency order, before any code); Layer 1, the implementer's mid-build declaration (`NEW_UI_ELEMENTS` / `DESTRUCTIVE_OPS`). Orthogonally, six **mechanical gates** are `PreToolUse` hooks that enforce *how* work is done: `force-subagent` (only a dispatched subagent may edit `sourceGlobs`; the main thread only orchestrates), `no-bom`, `tests-green`, `no-push`, `no-pr-to-protected`, `code-review-gate` (`docs/architecture.md#The mechanical gates`, `README.md` "Discipline is enforced by local hooks, not by trust"). The hard boundary: work merges **only** to the integration branch; the protected branch is never touched by the loop.
...
> Anything risky parks with a label instead of guessing (`README.md` "Anything risky… parks with a label instead of guessing"). Specifically: **never push to or open a PR against the protected branch** (mechanical `no-push` / `no-pr-to-protected` hooks); **adding a new third-party dependency** is a STOP-and-ask, parked `needs decision`, never an autonomous call (`docs/architecture.md` label taxonomy; the implementer's new-dependency PAUSE gate). A design gap or contradictory acceptance criteria parks `needs design`.
