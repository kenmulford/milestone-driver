...
> Three defense-in-depth gating **layers** catch design gaps before they reach the integration branch (`docs/architecture.md#the-layered-gating-model`): Layer 0 — proactive triage (design gaps + dependency order, before any code); Layer 1 — the implementer's mid-build declaration (`NEW_UI_ELEMENTS` / `DESTRUCTIVE_OPS`); Layer 2 — the visual-review gate (UI PRs held open for human sign-off). Orthogonally, six **mechanical gates** are `PreToolUse` hooks that enforce *how* work is done — `force-subagent` (only a dispatched subagent may edit `sourceGlobs`; the main thread only orchestrates), `no-bom`, `tests-green`, `no-push`, `no-pr-to-protected`, `code-review-gate` (`docs/architecture.md#the-mechanical-gates`, `README.md` "Discipline is enforced by local hooks, not by trust"). The hard boundary: work merges **only** to the integration branch; the protected branch is never touched by the loop.
