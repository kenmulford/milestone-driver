...
**Theme:** The visual-review hold is gone at every integration granularity — issue, wave, and milestone — so a UI issue auto-merges on green CI exactly like a logic issue, and the `visualHold` profile key is retired outright rather than deprecated.

### ✨ The visual-review hold is retired

| Issue | PR | What |
|---|---|---|
| #599 Remove the solve-issue visual-review hold and make auto-merge unconditional | — | `skills/solve-issue/visual-review-hold.md` is deleted and step 7 with it. `skills/solve-issue/SKILL.md` step 8 is now "Auto-merge on green": `gh pr merge --squash --delete-branch` runs for **every** issue, with no UI/non-UI branch in front of it, and the numbering gap stays rather than renumber because these headings are citation anchors. Visual capture moves into step 6.8 as optional, never-gating evidence guarded on the PR's diff matching `uiSurfaceGlobs`, so a logic-only issue no longer boots a server to publish an empty comment. `skills/solve-issue/milestone-clauses.md` folds its 6.7 row into 6.8, and `skills/solve-issue/resume-paths.md`'s open-PR resume path goes straight to auto-merge. The governed-file tables in `scripts/check-doc-toc.{sh,ps1}` and `scripts/check-size-budgets.{sh,ps1}` drop the deleted file, along with every fixture and `_expected/*.txt` golden that named it. |
| #600 Remove the needs-review hold from the solve-milestone waves and merge tail | — | The Phase 2 serial verified merge tail integrates **every** built-green branch, UI included, instead of excluding UI branches and leaving them labelled `needs review` for a human. `skills/solve-milestone/parallel-waves.md`, `sequential-loop.md`, `integration-granularity.md`, `changelog-authoring.md`, `milestone-granularity.md`, `SKILL.md`, and `skills/solve-issue/wave-clauses.md` drop the logic-only carve-out, so one wave PR carries the whole assembled Wave and one milestone PR carries the whole milestone. |
| #601 Retire the `visualHold` key and sweep the docs for the removed visual-review gate | — | Every documentation and config-schema reference to the per-issue, per-wave, and per-milestone visual-review hold is gone. `docs/architecture.md` drops from three gating layers to two and from three `visualCapture` invariants to two (the "never auto-merge a UI issue" invariant and its `visualHold: false` carve-out are deleted), and its wave-tail and milestone-granularity sections now integrate UI branches like any other. `docs/profile-schema.md` loses the `visualHold` key row, its Integration-tier entry, and its explanatory note; the `uiSurfaceGlobs` and `visualCapture` notes now describe capture as evidence-only with no gate to degrade to. `docs/consumer-setup.md` loses the visual-review-gate bullet, the whole `visualHold` section, and the checklist-tick caveat that assumed a held-open UI PR. `README.md`, `skills/notices.md`, and the plugin `description` drop the UI sign-off promise. |
| #607 Remove the remaining visual-hold call sites in milestone granularity, trello-sync, and the md-epic fan-out | — | `skills/solve-milestone/milestone-granularity.md` loses the whole `### The visualHold gate` section and its first-match-wins table; milestone-end step 4 merges on green CI unconditionally, and `### Red CI on the milestone PR` is untouched. `trello-sync.md` drops the not-ticked caveat, the open-`needs review`-UI-PRs summary field, and the `needs review`-is-not-a-park paragraph, so every merged issue ticks the same way. `skills/solve-issue/md-epic-fanout.md`'s outcome table goes from five outcomes to four (`done already` / `built this run` / `open, PR unmerged` / `parked with opens`): both visual-hold outcomes go, and the milestone-PR leg is rekeyed on the `needs review` label the red-CI handler applies rather than on a hold, its Note splitting first-match-wins across all five buckets `gh pr checks` emits. The `parked with opens` precedence bullet returns in its minimal form, a blocker label being the root block. `docs/architecture.md`'s fan-out report line names the same four. `wave-clauses.md`'s 6.7 row and `integration-granularity.md`'s two remaining `visualHold` citations are rewritten against the no-hold contract, and `solve-milestone/SKILL.md`'s profile row resolves `integrationGranularity` alone. The `needs review` label **stays** in `docs/architecture.md` and `skills/setup/SKILL.md`, its meaning rewritten from visual sign-off to CI-red, because `milestone-granularity.md` and `changelog-authoring.md` both still apply it and `setup` still provisions it. Stale claims outside every issue's declared scope go with them: `skills/setup/SKILL.md`'s `uiSurfaceGlobs` skip-consequence drops from three layers to two and its three `visualCapture` skip-consequences no longer promise a PR-open-for-human-test gate; `agents/design-reviewer.md` no longer sends screenshots to a gate that does not exist; `trello-sync.md`'s two call-site notes no longer restrict the tick to non-UI or logic issues; and the one-contract sentence in `wave-clauses.md`, `solve-issue/SKILL.md`, `parallel-waves.md`, and `solve-milestone/SKILL.md` is now stated one way, as `UI included`. A consistency pass over the same contract follows: `skills/output-style.md`'s Wave PR body shape lists every built-green issue, UI included, so a UI issue has the `### #<n>` sub-entry `hooks/code-review-gate.sh` requires at `gh pr create` and `gh pr merge`; `integration-granularity.md` gains a `## Phase 2 deltas` row skipping render capture under `"wave"` and `"milestone"`, where step 6.6 opened no per-issue PR to comment on; `version-bump.md` and `resume-paths.md` drop their non-UI carve-outs; the capture step is 6.8, not the retired 6.7, in `docs/architecture.md` and `docs/profile-schema.md`; the README diagram and the 1.16.0 pre-filter notice no longer promise a human merge gate; and both red-CI handlers now create the `needs review` label through the apply-time helper before applying it, so a repo that never provisioned it does not fail the edit. Visual capture's one hard precondition, a live per-issue PR to comment on, is now stated once as a guard in `skills/solve-issue/visual-capture.md` and cited by its callers rather than re-derived per granularity: `wave-clauses.md` had derived it wrong, letting a sequential `"wave"` run boot the render daemon and reach Publish with no PR to post to. |
...
- **All three holds are gone, not just the milestone one.** The per-issue hold (`solve-issue` step 7) and the per-wave hold (the merge tail's logic-only carve-out) are removed alongside the milestone PR's `visualHold` gate. No integration granularity holds a PR for a human look any more.
...
- **`uiSurfaceGlobs` still earns its place.** It drives design-lens triage and targets `visualCapture`. Screenshots are still attached when the render seam is configured; they are evidence on a PR that merges, never a gate.
- New gitignored artifact: none. New profile keys: none. Removed profile keys: `visualHold`.
- **Released entries below still name `visualHold`.** Changelog history is not rewritten to scrub a later-retired term, so a repo-wide `grep` for `visualHold` legitimately hits this file's pre-1.24.0 sections. Leave them.

### ⚖️ Post-run audit trail

Judgment-call PRs: none.

- Deferred by ownership, routed to #603: `.project/design-system.md` still states "the Layer-2 visual gate never fires". The whole `.project/` directory is human-owned, so this release edits none of it; #603 already carries `design-philosophy.md`'s two sites. Suggested replacement for the clause: "so the engine raises no UI issues here and no design-lens review runs."
...
- Ceiling state a contributor's next edit hits: `skills/solve-milestone/changelog-authoring.md` 46 bytes free, down from 162 when this release opened, and `skills/output-style.md` 53 bytes free; both WARN that a CRLF checkout would fail the row. `skills/solve-issue/visual-capture.md` sits at 794/800 words after taking the shared capture guard, its tightest axis. No ceiling was raised or lowered this release: the only governed-table edit is the removal of the deleted `skills/solve-issue/visual-review-hold.md` row and its exclusion bracket.
