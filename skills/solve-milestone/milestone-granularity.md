...
3. **Open one PR into `integrationBranch`.** Guard it for re-run safety exactly as step 6.7 does, checking `gh pr list --head "milestone-<number>-<slug>" --json number --jq '.[0].number // empty'` for an already-open PR before `gh pr create` and reusing the number it finds (`skills/solve-milestone/changelog-authoring.md § Check if a PR already exists for this branch (re-run safety)`). Base `<integrationBranch>`, never `protectedBranch`. Body:
   ...
   - **The simplify pass gets a sub-entry too, discovered by its own key.** When `git log <milestone-branch> --grep='^Simplify-Pass: ' --format=%H` finds a commit, add one `### /simplify` sub-entry carrying its `Code-Review:` block, de-indented the same way; it has no Decision Log (`skills/solve-milestone/simplify-pass.md § Land the findings`). That query cannot collide with the per-issue one above: the keys differ, and the pass writes no `Issue:` line at all, so it is absent from the issue enumeration and from step 5's close list by construction. No commit found means the pass found nothing or did not run, and nothing is added.
