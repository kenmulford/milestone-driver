...
| **Wave PR body** (`parallel-waves`) | — | the Wave's logic issues · **evidence** (per issue: its branch and the gates it passed on the wave branch) · **exactly one** anchored `## Code Review` heading, one `### #<n>` sub-entry per logic issue — a one-logic-issue Wave included, no special case (`skills/solve-milestone/milestone-granularity.md (aggregates these blocks under)`) |
