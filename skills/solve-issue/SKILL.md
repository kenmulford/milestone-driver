...
Dispatch `milestone-driver:planner` via the Agent tool, briefed with: the issue record path (step 1), the held step-0 result, the resolved build profile (a `risk:light` token under `light`), the profile keys `sourceGlobs`, the expanded `domainSkills`, `nonNegotiables` and `projectDocs`, the worktree path (the repo root standalone), the `common.md` path (omit when none is held), the resolved citations (omit any whose resolution was a no-op), `citationFormatPath` (the absolute path of `${CLAUDE_PLUGIN_ROOT}/skills/citation-format.md`, always passed), the packet path `<repo-root>/.milestone-config/.runtime/plans/issue-<n>.md`, and the packet contract path `${CLAUDE_PLUGIN_ROOT}/skills/solve-issue/build-packet.md`. The brief carries step 3's scratch-hygiene and command-shape rules, and states that the planner quotes each `.project/` anchor the issue cites that `common.md` does not carry into the packet's `## Rules`, a missing or renamed cited anchor being `STATUS: PARK` with `LABEL: blocked`, naming the anchor and file. Planner dispatches are capped at **2 per issue**, enforced by `hooks/dispatch-cap.sh`; a denied dispatch, or a return that is neither `PARK` nor `PLANNED` with a file at `PACKET:`, runs the park action with `blocked`, never a retry. A packet failing approval takes the one re-dispatch below.

- **`STATUS: PLANNED`** → hold the `PACKET:` path and approve it by three targeted extractions, never a whole-packet read. **Sections:** `grep -n '^## \|^Issue: \|^Base: \|^Worktree: ' <packet>` shows the three header lines and every section `skills/solve-issue/build-packet.md § Sections` requires, under its `### Omission` rules. **Base:** `Base:` equals `git -C <worktree> rev-parse HEAD`. **Citations:** each `### <path> (<anchor>)` heading that `grep -n '^### ' <packet>` lists resolves, `${CLAUDE_PLUGIN_ROOT}/scripts/resolve-citation.{sh,ps1} <worktree>/<path> <anchor>` exiting 0; a packet with none skips this check, the same no-op as the resolve block above.
  All three pass → the packet is approved, the **locked** architecture for this issue. Any failure → re-dispatch the planner once, same brief plus the failures one per line (a missing section, the `Base:` mismatch with both SHAs, an unresolved `path (anchor)` with the resolver's stderr), per `agents/planner.md (On a re-dispatch, fold the listed approval failures)`, and approve its new packet likewise. A second failure → run the park action with `blocked`, naming the failures; the planner cap of 2 is spent, so no further dispatch.
...
Verify the report honors the implementer contract: least-code / reuse-first, TDD red→green observed (or a `VERIFICATION (no test layer)` section when `unitTestCmd` is absent), verified citations where citable sources exist, a Decision Log, a `USER-FACING CHANGES` block (with `NEW_UI_ELEMENTS: yes|no`, `DESTRUCTIVE_OPS: yes|no`, and `POST_REVIEW_CHANGES: yes|no`), and **changes left uncommitted**. With `unitTestCmd` defined and `CLAUDE_HOOK_DISABLE_TESTS_GREEN` not `1`, the GREEN evidence is a scoped run of only the spec file(s) the implementer added or touched (`agents/implementer.md` rule 3) - this is expected, not a shortfall to flag; the full-suite run is step 4's Unit gate below. Then apply the declaration gates:
...
**🔴 GATE - implementer STOPPED:** `STATUS: STOPPED` naming no packet gap (architecture conflict, scope overrun, out-of-scope edit, or missing/ambiguous brief) → park describing the conflict, with `needs design` for a design/spec conflict, `needs decision` for an architecture call, or `blocked` for an otherwise-unresolvable gate. `PAUSED-FOR-APPROVAL` indicates a new-dependency case and routes to the gate above.
**A packet-gap stop re-plans:** a stop naming a packet gap, `agents/implementer.md (A fact the packet lacks, or a quoted excerpt that does not match its file)`, re-dispatches the planner, step 2's brief plus that fact, per `agents/planner.md (On a re-dispatch, fold the listed approval failures or the missing fact)`. `PLANNED` → approve per step 2's three checks; approved → re-dispatch the implementer with the new `PACKET:` path and no `model`; a failure parks `blocked`, no further planner dispatch. `PARK` → run the park action with its `LABEL` and `REASON`, no implementer re-dispatch. A denied planner or implementer dispatch parks `blocked`, never retried (step 4's shared loop).
...
3. **Retry.** A failing gate re-dispatches the implementer with the failure and `model: "opus"` - or parks directly if the failure reveals the plan is wrong (see Autonomy). A preflight fix routes through step 6.1's After a fix, before committing branch set - `${CLAUDE_PLUGIN_ROOT}/scripts/classify-delta.{sh,ps1}` picks the branch, and `code-changed` re-runs the unit gate, never `unitTestCmd` directly (`skills/solve-issue/post-fix-commit.md § After a fix`), and `/code-review`. Cap: 3 implementer dispatches per issue in total - the first build plus 2 fixes, shared across every gate and the step-6.1 review loop, enforced by `hooks/dispatch-cap.sh`. A denied dispatch is the park signal, never a retry.
