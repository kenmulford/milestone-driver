...
1. **Extract by model judgment over the `path (anchor)` shape - never a regex.** Apply `skills/citation-format.md`'s span and position tests to the issue body + acceptance criteria. A parenthetical following a path is **not** automatically a citation: resolving prose anyway yields either a **false drift report** (anchor absent, exit 1) or a **confident wrong answer** (the anchor happens to match).
