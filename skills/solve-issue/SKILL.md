...
1. **Review and resolve.** Run `/code-review` (`superpowers:requesting-code-review`) on the implementer's **uncommitted** changes, then resolve findings autonomously per the Autonomy model — do **not** pause to ask the operator about an in-scope finding:
   - **In-scope** (cosmetic, naming, style, local reversible refactor, missing/weak test): re-dispatch the implementer to fix it (the main thread cannot edit `sourceGlobs` — `force-subagent`); log it in the Decision Log. **Under the Light profile, fix only a Critical or Important finding**; a Minor one is accepted, not fixed. Heavy fixes every in-scope finding. Those severities are the reviewer template's (`skills/solve-milestone/parallel-waves.md (Critical / Important / Minor)`); a finding with none, including any from a reviewer that scores by confidence instead, counts as **Important**, so Light degrades safely to Heavy.
   ...
   **After a fix, before committing.** Evaluate the branches **in order**.
   - **No in-scope findings:** commit directly.
   - **Document-only** (every path the delta touches is `*.md` / a README / doc text, and **nothing** is under `sourceGlobs`): commit directly; no re-run needed (`tests-green` no-ops on doc-only, and `/code-review` need not be re-run for a doc-only fix).
   - Otherwise, run `${CLAUDE_PLUGIN_ROOT}/scripts/classify-delta.<sh|ps1> <repo-root>` (pwsh on Windows, bash elsewhere) and **take the branch it prints**. It writes `comment-only` or `code-changed` to stdout, one reason token to stderr on `code-changed`, and always exits 0. **The verdict is the script's, never the agent's**: do not re-derive it, and do not override it. What counts as a comment, and every input shape the rule was built against, live in `scripts/classify-delta.sh (comment-only delta classifier)` and `tests/classify-delta.cases.tsv`.
     - **`code-changed`:** re-run `unitTestCmd` if defined (skip if absent), then re-run `/code-review` — the fresh review must be the **last action before commit**. The procedure does not loop past a second clean review. `POST_REVIEW_CHANGES: yes` is the machine-checkable signal that the re-review is due **on a `code-changed` delta**; such a `sourceGlobs` change independently triggers it as a backstop, so a re-dispatch that changed source is always re-reviewed even if the field is `no`. On a `comment-only` delta neither the field nor the backstop fires.
     - **`comment-only`:** **stage in one Bash call, then `git commit` in the next** — never `git add … && git commit …` in a single call, where `tests-green` inspects the index before the `git add` has run and sees nothing staged. No `unitTestCmd` re-run, no `/code-review` re-run. The finding's resolution is still recorded in the **Code Review** section (step 6.3), same as any other fix.

   `hooks/tests-green.sh (Runs unitTestCmd when staged files touch sourceGlobs)` is a `PreToolUse` hook — it reads `git diff --cached --name-only` **before** the Bash call runs, so it sees only what an **earlier** call staged. Staged and committed in separate calls, it fires under every branch above and decides on **path**: a comment-only edit inside a `sourceGlobs` file runs the suite there and blocks on red at exit 2. **It is not a guaranteed second suite run.** It exits 0 having run nothing when `unitTestCmd` is absent from the profile (this repo's own configuration, and a supported one, per `docs/profile-schema.md (What command runs the unit tests? Absent →)`), when the staged tree still matches the last green run's stamp, when `jq` is absent, or when `CLAUDE_HOOK_DISABLE_TESTS_GREEN=1` is set. On the `comment-only` branch, which skips the orchestrator's own re-run, each of those is a case where no suite runs at all.

   **Cap: review→fix cycles, per build profile.** A cycle is one `/code-review` run plus the fix it triggers, and on a `code-changed` delta **the fresh review is the last action before commit**; Light never skips that review. Heavy: at most 2 cycles, and any in-scope finding surviving the 2nd fix parks, as before. Light: 1 cycle, and a 2nd only when the most recent review returned a **Critical or Important** finding. A Light Minor is disposed `accepted (rationale: <…>)` in the Code Review section (step 6.3); accepting changes no code, so neither the re-review trigger nor a 2nd cycle fires and that review is the fresh one. **Non-convergence parks only a finding its profile fixes**; the park-trigger bullet above is untouched and fires at any severity. To park: comment in the park-comment shape (`skills/output-style.md`) opening with `🔴 Parked — ` and the current diff state on the issue, apply `needs design` or `blocked` as appropriate (+ `in progress` if the branch has commits), preserve the branch, and return. A review that won't converge usually means the plan is wrong.
