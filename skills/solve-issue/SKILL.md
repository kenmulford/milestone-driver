...
|---|---|---|---|---|
| **Unit?** | `unitTestCmd` defined | Run `unitTestCmd`. | Shared (below) | Park `blocked` (or `needs design` if the plan is wrong). |
...
| **Preflight?** | `preflightCmd` defined (including the `"github-ci"` sentinel) | Run the literal `preflightCmd`; or, under `"github-ci"`, discover + run each CI-derived `STEP` (see below). | Shared (below) | Park `blocked` (or `needs design` if the plan is wrong). |
...
3. **Retry.** A failing gate re-dispatches the implementer with the failure - or **parks directly** if the failure reveals the plan is wrong (see Autonomy). A preflight fix routes through step 6.1's **After a fix, before committing** branch set - `${CLAUDE_PLUGIN_ROOT}/scripts/classify-delta.{sh,ps1}` picks the branch, and `code-changed` re-runs `unitTestCmd` and `/code-review`. **Cap: 3 implementer dispatches per issue in total - the first build plus 2 fixes, shared across every gate and the step-6.1 review loop, enforced by `hooks/dispatch-cap.sh`.** A denied dispatch is the park signal, never a retry.
...
4. **Skip cleanly.** Zero dispatches this run (e.g. a triage-blocker park before any dispatch) → skip the emission with one log line, no zero-value record. Writer script absent, or no `<usage>` figures surfaced → silent no-op, one log line.
