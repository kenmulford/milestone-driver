---
name: solve-issue
description: This skill should be used when the user invokes "/milestone-driver:solve-issue <n>", or asks to "solve issue <n>", "fix issue <n>", or "drive issue <n>" through the milestone-driver gated procedure. Runs one GitHub issue end-to-end as an orchestrator - triage, dispatch the planner and approve its build packet, dispatch the implementer subagent (TDD, citations), unit + E2E gates, code review, PR to the integration branch, auto-merge on CI green, then close - never authoring application or test code on the main thread.
---
...
1. Read the profile (see the plugin's `docs/profile-schema.md`).
   ...
   1.1.1. **One-time notices.** Immediately after the profile read: read `${CLAUDE_PLUGIN_ROOT}/skills/notices.md` and, in file order, evaluate each section whose `Skills` field includes `solve-issue`, applying that section's `Trigger` → `Text` → `Marker` → `Legacy fallback` mechanics. Never evaluate a section scoped only to `solve-milestone`.
...
The safety floor is **unconditional for both profiles**: triage (step 0) and the `tests-green`, `force-subagent`, and `dispatch-cap` hooks always run. Light relaxes ceremony only - it never skips verification.
...
- **Missing/renamed cited anchor** → a planner park (`agents/planner.md#Park`).
...
Dispatch `milestone-driver:planner` via the Agent tool, briefed with: the issue record path (step 1), the held step-0 result, the resolved build profile (a `risk:light` token under `light`), the profile keys `sourceGlobs`, the expanded `domainSkills`, `nonNegotiables` and `projectDocs`, the worktree path (the repo root standalone), the `common.md` path (omit when none is held), the resolved citations (omit any whose resolution was a no-op), `citationFormatPath` (the absolute path of `${CLAUDE_PLUGIN_ROOT}/skills/citation-format.md`, always passed), the packet path `<main-repo-root>/.milestone-config/.runtime/plans/issue-<n>.md` beside `common.md`, never inside a worktree, and the packet contract path `${CLAUDE_PLUGIN_ROOT}/skills/solve-issue/build-packet.md`. A run may add project facts to the brief, never work. The brief carries step 3's scratch-hygiene and command-shape rules; the planner's park cases are `agents/planner.md#Park`. Planner dispatches are capped at **2 per issue**, enforced by `hooks/dispatch-cap.sh`; a denied dispatch, or a return that is neither `PARK` nor `PLANNED` with a file at `PACKET:`, runs the park action with `blocked`, never a retry. A packet failing approval takes the one re-dispatch below.

- **`STATUS: PLANNED`** → hold the `PACKET:` path and approve it with check-packet, never a whole-packet read: run `${CLAUDE_PLUGIN_ROOT}/scripts/check-packet.{sh,ps1} <packet> <worktree>`, adding `light` under the light profile. Exit 0 → the packet is approved, the **locked** architecture for this issue. Nonzero → re-dispatch the planner once, same brief plus each `FAIL` line as one listed approval failure, per `agents/planner.md (On a re-dispatch, fold the listed approval failures)`, and approve its new packet likewise. A second failure → run the park action with `blocked`, naming the failures; the planner cap of 2 is spent, so no further dispatch.
- **`STATUS: PARK`** → run the park action with the returned `LABEL` and `REASON`.

Hold the last planner return's `DOMAIN_SKILLS_INVOKED` line for step 6.2.

### 3. Dispatch the implementer
Dispatch the profile's `implementerAgent` (default `milestone-driver:implementer`; a project-level override uses that agent's own name as-is) via the Agent tool with no `model` on the first build, so its frontmatter tier applies, orchestrating `superpowers:subagent-driven-development` + `superpowers:test-driven-development`. Brief it like a colleague walking in cold: the issue, the approved `PACKET:` path, the profile, the `common.md` path (omit when none is held), the resolved citations (omit any whose resolution was a no-op), the repo root (that issue's worktree path in a parallel-mode run), and `citationFormatPath`; under the `light` profile the brief must include a `risk:light` token. A run may add project facts to the brief, never work. Every implementer re-dispatch carries this brief, `PACKET:` included, plus the failure. Implementer dispatches are capped at **3 per issue** (step 4's shared loop), and a non-converging issue parks like any other. The brief must also carry this scratch-hygiene rule: write scratch only under a path named for that issue or that agent, never the shared scratchpad directory, and report what a probe printed rather than writing a probe file to read back later. The brief must also carry this command-shape rule: absolute paths only, and never change directory (`cd`, `pushd`, or a subshell); `git -C <dir>` for git, absolute file paths for `grep`, `cat`, `sed`, and `find`; a command needing a working directory (`unitTestCmd`, `preflightCmd`, `npm ci`) runs with the absolute worktree path as its cwd.
...
Verify the report honors the implementer contract: least-code / reuse-first, TDD red→green observed (or a `VERIFICATION (no test layer)` section when `unitTestCmd` is absent or under `risk:light`), verified citations where citable sources exist, a Decision Log, a `USER-FACING CHANGES` block (with `NEW_UI_ELEMENTS: yes|no`, `DESTRUCTIVE_OPS: yes|no`, and `POST_REVIEW_CHANGES: yes|no`), and **changes left uncommitted**. With `unitTestCmd` defined and `CLAUDE_HOOK_DISABLE_TESTS_GREEN` not `1`, the GREEN evidence is a scoped run of only the spec file(s) the implementer added or touched (`agents/implementer.md` rule 3) - this is expected, not a shortfall to flag; the full-suite run is step 4's Unit gate below. Then apply the declaration gates:
...
**🔴 GATE - implementer STOPPED:** `STATUS: STOPPED` carrying no `PACKET_GAP:` line (architecture conflict, scope overrun, out-of-scope edit, or missing/ambiguous brief) → park describing the conflict, with `needs design` for a design/spec conflict, `needs decision` for an architecture call, or `blocked` for an otherwise-unresolvable gate. `PAUSED-FOR-APPROVAL` indicates a new-dependency case and routes to the gate above.

**A packet-gap stop re-plans:** a stop carrying a `PACKET_GAP:` line, `agents/implementer.md (A fact the packet lacks, or a quoted excerpt that does not match its file)`, re-dispatches the planner, step 2's brief plus that fact, per `agents/planner.md (On a re-dispatch, fold the listed approval failures or the missing fact)`. Planner dispatches already at 2 (step 2 re-dispatched) → park `blocked` naming the gap, no dispatch. `PLANNED` → approve per step 2's check-packet run; approved → re-dispatch the implementer with the new `PACKET:` path and no `model`, then apply step 3's checks to its return; a failure parks `blocked`, no further planner dispatch. `PARK` → run the park action with its `LABEL` and `REASON`, no implementer re-dispatch. A denied planner or implementer dispatch parks `blocked`, never retried (step 4's shared loop).
...
Unit, E2E, and preflight share one shape - **act → verify → retry → park**. `/code-review` is listed for visibility only (always-on, hence no trailing `?` in its row): it keeps its own step-6.1 procedure and is not iterated by the shared loop below.

| Gate | Applicability | `act` | Cap | Park / escape policy |
|---|---|---|---|---|
| **Unit?** | `unitTestCmd` defined | Run `${CLAUDE_PLUGIN_ROOT}/scripts/unit-gate.<sh\|ps1> <repo-root>` (pwsh on Windows, bash elsewhere), never `unitTestCmd` directly - it stages, runs `unitTestCmd`, and on green writes the same stamp `hooks/tests-green.<sh\|ps1>` reads, so the commit-time hook does not re-run a suite this gate already proved green on this exact tree. | Shared (below) | Park `blocked` (or `needs design` if the plan is wrong). |
| **E2E?** | Issue touches a UI surface and `e2eTestCmd` defined | Bug: run a targeted subset that proves the fix. Feature: implementer authors new E2E tests covering reasonable user stories, then runs them. Both against the profile's `e2eEnv`. | Shared (below) | Verified by other means (a DB assertion + an attached screenshot confirming the feature) → quarantine the flaky test, proceed, log the quarantine in the PR's Code Review section, apply `judgment call`. Not otherwise verified → park `blocked`. |
| **`/code-review`** | Unconditional: always runs, no applicability flag | See step 6.1, its own in-scope-fix / park-trigger classification | Step 6.1's classifier verdict, per `skills/review-depth.md § The ladder` | Step 6.1's own park-trigger classification (architecture deviation, shared-contract change, new dependency, out-of-scope edit, unmetable gate, material ambiguity), plus its second-cycle park rule |
| **Preflight?** | `preflightCmd` defined as a literal command | Run the literal `preflightCmd`. | Shared (below) | Park `blocked` (or `needs design` if the plan is wrong). |

**Preflight's `"github-ci"` sentinel mode.** When `preflightCmd` is the reserved sentinel `"github-ci"`, the Preflight row above does not apply: no local command runs, no cap, no park. The PR's own CI is the preflight, gated by step 8's existing wait-for-green merge under whichever granularity is in effect (the per-issue PR, the wave PR, or the milestone PR), each with its existing red-CI handling. Read `${CLAUDE_PLUGIN_ROOT}/skills/solve-issue/preflight-github-ci.md` for the record.
...
3. **Retry.** A failing gate re-dispatches the implementer (step 3's re-dispatch brief) with the failure and `model: "opus"` - or parks directly if the failure reveals the plan is wrong (see Autonomy). A preflight fix routes through step 6.1's After a fix, before committing branch set - `${CLAUDE_PLUGIN_ROOT}/scripts/classify-delta.{sh,ps1}` picks the branch, and `code-changed` re-runs the unit gate, never `unitTestCmd` directly (`skills/solve-issue/post-fix-commit.md § After a fix`), and `/code-review`. Cap: 3 implementer dispatches per issue in total, however split, shared across every gate and the step-6.1 review loop, enforced by `hooks/dispatch-cap.sh`. A denied dispatch is the park signal, never a retry.
4. **Park.** Still failing when the budget is spent → apply that gate's park/escape policy from the table above; the failed gate and its real output are the park comment's evidence slot. E2E's row is the one exception: verified-by-other-means quarantines and proceeds instead of parking.

**Call sites.** Unit and E2E run back-to-back immediately after the implementer dispatch (step 3). Preflight fires as the concluding action of step 6.1, after the `/code-review` loop converges and before version bump/commit.

### 6. Review → integrate → close

**Coherence review (before the final `/code-review`).** An optional read-only pass over the implementer's uncommitted diff. Run the review-depth classifier first (`skills/review-depth.md § Running the classifier`, `<repo-root>` as root): a `shallow` verdict skips this pass. Otherwise run it only when the `coherenceReviewAgent` - profile key, default-filled to `milestone-coherence-reviewer:coherence-reviewer` - is both present (dispatchable in this session) and configured; absent/unavailable OR explicitly unconfigured → silently skip: no error, no prompt, at most one log line. When it runs, read `${CLAUDE_PLUGIN_ROOT}/skills/solve-issue/coherence-review.md` and follow its `### The pass`.

1. **Resolve review depth, then review and resolve.** Read `${CLAUDE_PLUGIN_ROOT}/skills/review-depth.md` and follow it, passing `<repo-root>` as the classifier's root: it holds the verdict-to-effort mapping, the per-verdict cycle cap, the classifier's fail-open behavior, the re-classify-before-a-second-cycle rule, and the second-cycle park. Then run `/code-review` (`superpowers:requesting-code-review`) at that effort on the implementer's uncommitted changes, resolving findings autonomously per the Autonomy model, never pausing to ask the operator about an in-scope finding:
   - **In-scope** (cosmetic, naming, style, local reversible refactor, missing/weak test): take the pre-fix snapshot first - `${CLAUDE_PLUGIN_ROOT}/scripts/classify-delta.<sh|ps1> --snapshot <repo-root>` (pwsh on Windows, bash elsewhere), holding the tree hash it prints; on nonzero exit hold nothing and still re-dispatch, the post-fix branch set names the move. Then re-dispatch the implementer (step 3's re-dispatch brief) to fix it with `model: "opus"` (the main thread cannot edit `sourceGlobs` - `force-subagent`); log it in the Decision Log. That hash is what the post-fix branch set below classifies against. Which findings the build profile fixes, and how a finding carrying no severity is scored, are `skills/review-depth.md § Which findings get fixed`.
   - **Park trigger** (architecture deviation; a shared contract/interface/schema change; a new dependency; edits outside the issue's file scope; an unmetable gate; material ambiguity): park with `needs design`, `needs decision`, or `blocked`. Do not commit.

   **The orchestrator runs this review itself, dispatching the reviewers directly as leaves** (`docs/architecture.md` → `## Dispatch topology`), and never dispatches an agent that itself runs `/code-review`. Reviewing several issues concurrently pins the shape to exactly one reviewer leaf per issue (`skills/solve-milestone/parallel-waves.md § Parallel mode - Phase 1: concurrent stage dispatch`, Stage B). The gate is unconditional: its `### 4. Verification gates` row carries no applicability flag, and `hooks/code-review-gate.sh` denies a `gh pr create` / `gh pr merge` whose PR body carries no `## Code Review` section. The brief must also carry the step-3 scratch-hygiene and command-shape rules and `citationFormatPath`.
...
2. Assemble the Decision Log from the implementer's report for the PR body. Its slots are the Decision Log entry shape in `skills/output-style.md` - choice · rationale · citation · rejected alternatives, one entry per line. Carry `planner DOMAIN_SKILLS_INVOKED: <value>` and `implementer DOMAIN_SKILLS_INVOKED: <value>`, one line each, `none` recorded as `none`.
   ...
   **Check every `path (anchor)` citation via `${CLAUDE_PLUGIN_ROOT}/scripts/resolve-citation.{sh,ps1}` before posting.** Resolves → post verbatim. Fails → correct that citation from code, or set it to `none` if no anchor exists. No other rewriting.
   ...
   Post the block on the issue as the Decision Log comment opening `📋 Decision Log`, the implementer's CITATIONS block after it - the record the aggregate PR bodies' choice-only shape cites (`skills/output-style.md (Choice-only Decision Log)`). Probe `gh issue view <n> --json comments` for that opener: found, replace that comment's body (`gh api -X PATCH repos/{owner}/{repo}/issues/comments/<id> -F body=@<file>`), so a re-drive's entries land on the cited record; absent, create it (`gh issue comment <n>`). Never a second comment. **Ungated**: `none` never blocks, parks, or fails anything.
3. **Assemble the Code Review section** for the PR body. Record whether `/code-review` ran, the finding count and severity per run (the 1st, and the 2nd if a re-review ran), and each finding's resolution (re-dispatched and resolved / accepted with rationale / triggered park). Its slots are the `## Code Review` section shape in `skills/output-style.md`; the evidence slot is the ref each finding named (per `skills/citation-format.md`; at zero findings, the effort level used). Template:
...
6. Push the feature branch and open a PR with `--base <integrationBranch>` (never `protectedBranch` - enforced by the `no-push` / `no-pr-to-protected` hooks and GitHub branch protection). Put the Decision Log and the **Code Review** section in the PR body. Add a `judgment call` label if any borderline autonomous call was made. Record its number (`gh pr view --json number --jq .number`) as step 8's `<pr-number>`.
8. **Auto-merge on green.** Once CI is green, squash-merge for every issue, no UI/non-UI branch in front of it. This replaces the human-choice step of `superpowers:finishing-a-development-branch`.

   **Every squash merge in this plugin passes an explicit subject and body**, here and at every squash site citing this section.

   ```bash
   body="Decision Log and Code Review: PR #<pr-number>" \
   ...
   **One Bash call, `&&`-joined** - a variable does not survive a call boundary, and an empty title aborts before the merge. The title is read at merge time, never held, and every value these strings interpolate is a variable, so a `$`, a backtick, or a `"` in one is safe. `$body` is `"Decision Log and Code Review: PR #<pr-number>"` where the PR body carries a Decision Log, `"Code Review: PR #<pr-number>"` where it carries none.

   **`<pr-number>` is written into the command as literal digits, never a variable.** The merge gate's `pr_arg` regex reads the first token after `merge` and runs `gh pr view` on it (`hooks/code-review-gate.sh (gh pr view "$pr_arg")`); on a non-literal token that view fails and the gate falls open.
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/output-style.md` - this plugin's output contract. `## Terminal output` governs what this skill prints (including the `## Output spec` template rule); `## GitHub-facing prose`, `## When prose is the correct form`, and `## Evidence slots` govern every issue comment, PR comment, Decision Log, and PR body. The two surfaces are distinct - terminal rules never reach GitHub. **Plain English.** Write every response, document, and GitHub issue, milestone, comment, and PR body in plain, concise English. Never include hypothesis, conjecture, or defensive text.
