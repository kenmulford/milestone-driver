...
2. Assemble the **Decision Log** from the implementer's report for the PR body, and post the citations on the issue for review (`gh issue comment <n>`). Its slots are the Decision Log entry shape in `skills/output-style.md` - choice · rationale · citation · rejected alternatives, one entry per line. Carry the implementer's `DOMAIN_SKILLS_INVOKED` value as one more line. **Ungated**: `none` never blocks, parks, or fails anything.
