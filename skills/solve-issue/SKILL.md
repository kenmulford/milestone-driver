...
Name the issue's cited `.project/` anchors **once, here in the orchestrator**, so the implementer reads exactly those sections and never a whole doc. Runs after the build profile resolves and before ### 3. Dispatch the implementer, adding inputs to the step-3 brief.
...
3. **Pass the anchors, not the sections.** Put the cited anchors, plus plausibly-relevant sibling section names, into the ### 3 brief as the cited `.project/` anchors, with the absolute path of `${CLAUDE_PLUGIN_ROOT}/scripts/read-doc-section.{sh,ps1}`. The implementer reads each with that primitive (`read-doc-section.<sh|ps1> <doc-path> <anchor-text>`, `<anchor-text>` the heading text without leading `#`s; it prints only that section) and keeps its own `Read`/grep tools for any additional anchor. Bias toward over-inclusion in the list: under-retrieval is the real risk. Never inline a section.
...
5. **Name the prose contract.** Put the absolute path of `${CLAUDE_PLUGIN_ROOT}/skills/output-style.md` into the ### 3 brief as the prose contract path, naming the four sections the agent reads: `## GitHub-facing prose`, `## When prose is the correct form`, `## Evidence slots`, `## The two anti-criteria`. Never paste them. They govern the implementer's Decision Log and every other GitHub-facing shape its report feeds; the agent's own `## Communication style` may specialize them, never replace them. Sub-steps 4 and 5 apply identically under both the `light` and `heavy` build profiles - never skipped or altered for `risk:light`. The code-comment rule is `agents/implementer.md` rule 8; the brief does not restate it.
...
- **Missing/renamed cited anchor** → the implementer's primitive call fails loud (non-zero exit, naming the anchor + file on stderr); it STOPs and reports rather than guessing the section, and that report parks per the Autonomy model.
- **Absent/failed file-index resolver** → no-op: no file index, no error - not fail-loud; nothing cites the index by name.
- **Absent or unreadable `skills/output-style.md`** → no-op: the brief names no path, no error; each agent's own `## Communication style` is its only prose rule.
