...
4. **Drive each resolved milestone sequentially, in listed order** — never concurrently; a later milestone may depend on an earlier one's merged code:
   ...
   - **A systemic failure inside a driven `solve-milestone`** (`gh auth`, a broken `integrationBranch`, missing tooling — `skills/solve-milestone/SKILL.md:448-451`) halts the **whole fan-out loop**, not just the current milestone — later milestones cannot be driven safely either.
...
6. **Aggregate summary**, one row per milestone — mirroring `solve-milestone`'s own run-complete reporting shape (Template 3, `skills/solve-milestone/SKILL.md:497`; the Final summary content requirements, `:530`). Classify each driven milestone from ground truth after driving, not from the driven run's own narrative (the same re-derive-over-handback posture already used at the Wave barrier, `skills/solve-milestone/SKILL.md:369`, `:383-387`):
