...
The branch that reaches this file belongs to the caller, which resolves `integrationGranularity` from the profile at step 1 and only then reads it (`skills/solve-issue/SKILL.md (Resolved from the profile at step)`). Reaching this file therefore means the key resolved to `"wave"`. `### Clauses` below is the complete set of per-step deltas that mode applies to the caller's pipeline, and every one of them applies to a **non-UI** issue only — settle it **before 6.2** from `git diff <integrationBranch>...HEAD --name-only` ∩ `uiSurfaceGlobs` (`skills/solve-milestone/parallel-waves.md (independently from)`), never from a PR's changed files, 6.6 having opened none: a UI issue runs the pipeline byte-unchanged, held per-issue for visual sign-off (`skills/solve-milestone/integration-granularity.md (Logic-only carve-out)`). The wave branch, the one wave PR, the explicit close, and its disposition are `${CLAUDE_PLUGIN_ROOT}/skills/solve-milestone/integration-granularity.md § "wave"`'s, not restated here.

### Clauses

| Step | Clause |
|---|---|
| 6.2 | There is no per-issue PR body: the Decision Log is written into the step-6.5 commit in the trailer shape `skills/solve-milestone/milestone-granularity.md (Subject line, a blank line, the Decision Log summary)` defines. The citation post to the issue (`gh issue comment <n>`) is unchanged. |
