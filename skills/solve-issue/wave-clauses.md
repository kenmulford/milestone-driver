...
| 6.7 | **No-op**: the glob match test cannot fire on the non-UI issues these clauses govern, and nothing defers upward, `visualHold` being `"milestone"`-only (`docs/profile-schema.md (Should the single milestone PR wait)`). |
