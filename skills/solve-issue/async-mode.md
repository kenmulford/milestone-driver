...
- **No PushNotification.** Dispatched leaves do not send notifications: PushNotification is confirmed absent from subagent tool registries. The orchestrator emits at its own chunk boundaries (parks, wave completions, run complete/halt), which it can do because it ran the gates itself.
