...
**Tier: Visual Capture** (optional; presented only when **both** gates pass — (a) a visual-capture signal was detected in Phase 1 (the signals are listed in the Phase-1 detection row), **and** (b) `uiSurfaceGlobs` was captured in the Triage / Visual tier above. The two gates are **independent and AND'd**: a native UI stack (MAUI, WPF) has UI surfaces — so `uiSurfaceGlobs` is captured — but has no server or URL to poll and should omit `visualCapture` entirely (`docs/profile-schema.md:136`). If either gate fails, skip the whole tier silently, exactly as the E2E tier is skipped when no E2E signals are detected.)
