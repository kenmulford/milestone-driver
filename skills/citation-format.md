...
loud on a miss; `skills/triage/SKILL.md (Pass the anchors, not the sections)`
names that resolver once per run over the anchors an issue cites. `path § Heading` is the
